...
def get_credential() -> Credential:
    ...
    # 尝试从配置中获取，如果没有则尝试从 bilibili.json 加载
    ...
    if not sessdata or not bili_jct or not buvid3:
        try:
            # 尝试从不同位置读取配置文件
            possible_paths = [
                "../../../bilibili.json",  # 相对于 utils/utils.py 的路径 (原始路径)
                "../../bilibili.json",     # 另一种可能的路径 (原始路径)
                "../bilibili.json",        # 第三种可能的路径 (原始路径)
                "./bilibili.json",         # 当前目录
                os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "bilibili.json"),  # 绝对路径 (原始路径)
                os.path.join(os.path.dirname(os.path.dirname(__file__)), "bilibili.json"),  # 相对于 utils 目录的上级目录 (blivelisten 目录)
            ]
            
            bilibili_config = None
            for path in possible_paths:
                abs_path = os.path.abspath(path)
                if os.path.exists(abs_path):
                    with open(abs_path, 'r', encoding='utf-8') as f:
                        bilibili_config = json.load(f)
                        break
            
            if bilibili_config:
                # 更新配置，优先使用配置文件中的值
                sessdata = bilibili_config.get('SESSDATA') or config.get("SESSDATA")
                bili_jct = bilibili_config.get('BILI_JCT') or config.get("BILI_JCT")
                buvid3 = bilibili_config.get('BUVID3') or config.get("BUVID3")
                
                # 同时设置到 config 中，以便后续调用
                if sessdata:
                    config.set("SESSDATA", sessdata)
                if bili_jct:
                    config.set("BILI_JCT", bili_jct)
                if buvid3:
