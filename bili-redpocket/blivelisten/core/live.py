...
# 注意：不要在这里配置 logger，因为 main.py 已经配置了
# 如果在这里调用 logger.remove() 会移除 main.py 添加的文件日志处理器
...
class LiveRoom:
    ...
    async def send_red_pocket(self,
                              red_pocket_id: int,
                              danmu_id: int,
                              danmu_msg: str,
                              context_type: int,
        ...
        Args:
            ...
            danmu_msg: 弹幕消息
            context_type: 上下文类型
            parent_area_id: 父分区ID
            area_id: 分区ID
            duration: 持续时间，默认 300
            platform: 平台，默认 "pc"
            build: 版本号，默认 0
            visit_id: 访问ID，默认为空
        ...
        api = API["operate"]["send_red_pocket"]
        data = {
            "ruid": await self.__get_ruid(),
            "room_id": self.room_display_id,
            ...
            "context_id": self.room_display_id,
            "parent_area_id": parent_area_id,
            "area_id": area_id,
            "platform": platform,
            "mobile_app": "pc",
            "build": build,
            "live_statistics": '{\"pc_client\":\"pcWeb\",\"trackid\":\"-99998\",\"jumpfrom\":\"85001\",\"room_category\":\"0\",\"official_channel\":{\"program_room_id\":\"-99998\",\"program_up_id\":\"-99998\"}}',
            "statistics": '{\"platform\":5,\"pc_client\":\"pcWeb\",\"appld\":100}',
            "duration": duration,
            "visit_id": visit_id
        }
...
class LiveDanmaku(AsyncEvent):
    ...
    async def __send_verify_data(self, ws: ClientWebSocketResponse, token: str):
        ...
        queue_uuid = ''.join(random.choice(chars) for _ in range(8))
        verify_data = {"uid": self.__uid, "roomid": self.__room_real_id, "protover": 3,
                       "buvid": config.get("BUVID3"), "support_ack": True, "queue_uuid": queue_uuid,
